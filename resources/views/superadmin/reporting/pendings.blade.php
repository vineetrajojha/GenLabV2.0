        ...
        <div class="card-header d-flex flex-wrap align-items-center justify-content-between gap-2">
            <div class="search-set d-flex align-items-center gap-3 w-100" style="max-width:50%;">
                ...
                <div class="mode-toggle-group d-flex align-items-center flex-shrink-0">
                    <a href="{{ route('superadmin.reporting.pendings', array_filter(['mode'=>'reference','department'=>request('department'),'search'=>request('search'),'month'=>request('month'),'year'=>request('year'),'overdue'=>request('overdue')])) }}" class="mode-toggle {{ $mode==='reference' ? 'active' : '' }}">By Reference No</a>
                    <a href="{{ route('superadmin.reporting.pendings', array_filter(['mode'=>'job','department'=>request('department'),'search'=>request('search'),'month'=>request('month'),'year'=>request('year'),'overdue'=>request('overdue')])) }}" class="mode-toggle {{ $mode==='job' ? 'active' : '' }}">By Job Order No</a>
                    @php
                        $toggleParams = [
                    ...
                    @endphp
                    <a href="{{ route('superadmin.reporting.pendings', array_filter($toggleParams, fn($v) => !is_null($v) && $v !== '')) }}" class="mode-toggle {{ request('overdue') ? 'active' : '' }}">Out of Expected Date</a>
                </div>
            </div>
