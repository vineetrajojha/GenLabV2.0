...
<div class="content">
    ...
    <div class="card mb-3">
        <div class="card-body">
            <form method="GET" action="{{ route('superadmin.reporting.generate') }}" class="row g-2 align-items-end">
                <div class="col-sm-4">
                    ...
                    <table class="table table-striped align-middle">
                        ...
                        <tbody>
                            @forelse($items as $item)
                                <tr>
                                    <td>
                                        <input type="checkbox" name="items[]" value="{{ $item->id }}" class="row-check">
                                    </td>
                                    ...
                                    <td>
                                        <div class="d-flex align-items-center gap-2 format-cell" data-item-id="{{ $item->id }}">
                                            <input type="hidden" name="format[{{ $item->id }}]" class="selected-format-id">
                                    ...
                                    <td>
                                        @if($item->received_at)
                                        <button type="button" class="btn btn-sm btn-primary create-btn disabled-create edit-format-btn" disabled data-item-id="{{ $item->id }}">Create</button>
                                        @else
...
(function(){
    ...
    const formatsEndpoint = @json(route('superadmin.reporting.report-formats.index')) + '?ajax=1';
    ...
    function openEdit(formatId, btn){
        ...
        (async ()=>{
            try {
                ...
                const url = @json(url('superadmin/reporting/report-formats')) + '/' + formatId + '/content';
    ...
    function attachEditFormHandlers(formatId, triggerBtn){
        ...
        formEl.addEventListener('submit', async function(ev){
            ...
            if(editor){
                ...
                try {
                    const resp = await fetch(@json(url('superadmin/reporting/report-formats')) + '/' + formatId + '/content', {
                        method:'POST',
        ...
        if(previewBtn){
            previewBtn.addEventListener('click', function(){
                window.open(@json(url('superadmin/reporting/report-formats')) + '/' + formatId + '/export-pdf','_blank');
            });
        }
    }
})();
