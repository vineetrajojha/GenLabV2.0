...
<div class="content">
    ...
    <div class="d-flex gap-2">
            <a href="{{ route('superadmin.labanalysts.index') }}" class="btn btn-outline-secondary">Back</a>
    ...
    <div class="card-body">
        ...
        #format-content { display: inline-block; transform-origin: top left; }
            ...
            <input type="hidden" id="gl-file" value="{{ $encoded }}">
            <input type="hidden" id="gl-reference" value="{{ $reference_no ?? '' }}">
            <input type="hidden" id="gl-jobcard" value="{{ $job_card_no ?? '' }}">
...
(async function(){
    ...
    const previewUrl = `{{ route('superadmin.labanalysts.preview') }}` + '?' + qs.toString();
    ...
    document.getElementById('pdf-btn')?.addEventListener('click', () => {
        ...
        const url = `{{ route('superadmin.labanalysts.pdf') }}` + '?' + params.toString();
