...
@section('title', 'Cheque Alignment Editor')
...
<div class="content container-fluid">
  <div class="page-header">
      <div class="row align-items-center">
          <div class="col">
              <h3 class="page-title">Cheque Alignment - {{ $bank->bank_name }}</h3>
          </div>
      </div>
  </div>
      ...
      <div class="card">
        ...
        <div class="card-body">
          <div id="chequeCanvas" class="cheque-canvas">
            <img id="chequeBg" src="{{ asset('storage/' . $bank->cheque_image_path) }}" alt="Cheque" draggable="false" />
            @php
              $defaults = [
            ...
            {{-- Text fields (including single date) --}}
...
(function(){
  ...
  $('#btnSave').on('click', function(){
    ...
    $.ajax({
      method: 'POST',
      url: "{{ route('superadmin.cheque-templates.store', $bank->id) }}",
      data: { positions: payload, delete_fields: deleteFields, _token: '{{ csrf_token() }}' },
    }).done(function(){
      if (window.Swal) {
