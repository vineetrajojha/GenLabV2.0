...
(function(){
    ...
    const pusherKey = @json(config('broadcasting.connections.pusher.key') ?? env('PUSHER_APP_KEY')) || '';
    const pusherCluster = @json((config('broadcasting.connections.pusher.options.cluster') ?? config('broadcasting.connections.pusher.cluster') ?? env('PUSHER_APP_CLUSTER'))) || 'mt1';
