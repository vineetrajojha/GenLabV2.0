            ...
            <div class="card-header d-flex flex-wrap align-items-center justify-content-between gap-2">
                ...
                <div class="search-set">
                    <form method="GET" action="{{ route('superadmin.bookingInvoiceStatuses.index', $department?->id) }}" class="d-flex gap-2">
                        ...
                        <select name="marketing_person" class="form-control" style="{{ $lockedMarketingCode ? 'pointer-events: none;' : '' }}">
                            <option value="">Select Marketing Person</option>
                            @foreach($marketingPersons as $mp)
                                <option value="{{ $mp->user_code }}" {{ ($lockedMarketingCode ? $lockedMarketingCode == $mp->user_code : request('marketing_person') == $mp->user_code) ? 'selected' : '' }}>
                                    {{ $mp->label }}
                                </option>
                            @endforeach
                        </select>
                        @if($lockedMarketingCode)
                            <input type="hidden" name="marketing_person" value="{{ $lockedMarketingCode }}">
                        @endif
