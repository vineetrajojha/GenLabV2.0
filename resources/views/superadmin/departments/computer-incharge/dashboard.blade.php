...
@php
    $pageTitle = 'Computer Incharge Dashboard';
    $metrics = collect($payload['metrics'] ?? [])->pluck('value', 'label');
    $insightMessage = $payload['insights']['message'] ?? 'Control repository hygiene and stay ahead of operational approvals.';
@endphp
    ...
    <div class="content">
        <div class="d-flex align-items-center justify-content-between flex-wrap gap-3 mb-3">
            <div>
                ...
                <p class="text-muted mb-0">Systems and documentation pulse for {{ $user->name ?? 'your operations' }}.</p>
            </div>
            <div class="d-flex flex-wrap gap-2">
                <a href="{{ route('superadmin.documents.index') }}" class="btn btn-primary d-flex align-items-center gap-2">
                    <i class="ti ti-folders"></i>
                    <span>Document Centre</span>
                </a>
                <a href="{{ route('superadmin.approvals.index') }}" class="btn btn-outline-secondary d-flex align-items-center gap-2">
                    ...
                    <div class="card-body">
                        ...
                        <ul class="list-unstyled mb-0">
                            <li class="d-flex align-items-center justify-content-between border-bottom py-2">
                                ...
                                <span class="badge bg-info text-dark">{{ $metrics->get('Documents Today', 0) }}</span>
                            </li>
                            <li class="d-flex align-items-center justify-content-between border-bottom py-2">
                            ...
                            <li class="d-flex align-items-center justify-content-between border-bottom py-2">
                                ...
                                <span class="badge bg-danger">{{ $metrics->get('Unassigned Samples', 0) }}</span>
                            </li>
                            <li class="d-flex align-items-center justify-content-between pt-2">
                                <span>Total repository size</span>
                                <span class="badge bg-primary">{{ $metrics->get('Repository Size', 0) }}</span>
                            </li>
                        </ul>
                    </div>
                </div>
